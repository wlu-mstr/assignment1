...
class Node(object):
    ...
    def __str__(self):
        ...
        return "name=%s, op=%s, const_attr=%s" %(self.name, self.op, self.const_attr)
...
def gradients(output_node, node_list):
    ...
    node_to_output_grad = {}
    # Traverse graph in reverse topological order given the output_node that we are taking
    #  gradient wrt.
    ...
    for node in reverse_topo_order:
        ...
        if not node.inputs or not input_grads:
            continue

        for ipt_node, ipt_grad in zip( node.inputs, input_grads):
            node_to_output_grads_list[ipt_node].append(ipt_grad)
